...
def generate_report(vulnerability_type, url, vulnerability_detected, payload):
    ...
    if vulnerability_type == "Union-based SQLi":
        ...
        if vulnerability_detected:
            c.drawString(50, 540, f"Payload: {payload}")
            c.drawString(50, 510, "Risk: Medium")
        else:
            c.drawString(50, 540, "Tested every payload available, No vulnerability Found")
        # Add given content
        content = [
        ...
        if vulnerability_detected:
            c.drawString(50, 540, f"Payload: {payload}")
            c.drawString(50, 510, "Risk: Medium to High")
        else:
            c.drawString(50, 540, "Tested every payload available, No vulnerability Found")
        # Add given content
        content = [
...
def main():
    ...
    if selected == "Home":
        ...
        st.write(
            "<span style='font-size: 17px; font-family: Arial;'>Auto-wt Tool is a web application security testing tool designed to automate the process of identifying and assessing vulnerabilities in web applications. It provides users with a user-friendly interface to initiate security tests and generate reports based on the detected vulnerabilities.</span>\n\n"
            "<span style='font-size: 17px; font-family: Arial;'>With the Auto-wt Tool, users can conveniently perform security testing without extensive knowledge of complex security testing methodologies or manual vulnerability scanning techniques. The tool automates various stages of the testing process, from input validation to payload injection and result analysis.</span>",
            unsafe_allow_html=True
        )
        st.markdown(
            '<p style="width: 100%; color: red; text-align: center;font-family: Arial">NOTE: Please ensure that you have proper authorization and permission before conducting any security testing on real-world websites. Use this system for educational purposes.</p>',unsafe_allow_html=True)
        st.write("\n\n")
        st.write("\n\n")
        st.write("\n\n")
        st.write(
            "<span style='font-size: 14px; font-family: Arial;'>For any inquiries or support, please reach out to us at:</span>\n\n"
            "<span style='font-size: 14px; font-family: Arial;'>Email: naveenkrishnakv04@gmail.com , mailtoaadithyu@gmail.com</span>\n\n",
            unsafe_allow_html=True
        )

    if selected == "Tool":
        st.markdown("<h1 style='text-align: center;font-family: Frutiger;'></h1>", unsafe_allow_html=True)
